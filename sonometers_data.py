...
def agregar_dia_semana_numerico(df):
    ...
    # Assign timezone (handling ambiguities with 'NaT' if necessary)
    df[date_column] = df[date_column].dt.tz_localize(bilbao_tz, ambiguous='NaT', nonexistent='NaT')

    # Assign weekday as a number (0=Monday, 6=Sunday)
    df['week_day'] = df[date_column].dt.weekday

    # Remove timezone before continuing
...
def main():
    ...
    start_date = "20250401"  # Start date in YYYYMMDD format
    end_date = "20250402"    # End date in YYYYMMDD format
    
    # Define output file name for the download
    output_file = f"bilbao_sonometers_data_new.csv"
